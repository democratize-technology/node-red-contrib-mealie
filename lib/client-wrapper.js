...
const retryPolicy = retry(
    ...
    }),
    {
        ...
        backoff: new ExponentialBackoff({
            ...
            jitter: false        // Disable jitter in tests for predictability
        })
    }
);
...
async function executeWithClient(config, operation, node, msg) {
    try {
        return await retryPolicy.execute(async ({ attempt }) => {
            if (attempt > 1) {
                node.log(`Retry attempt ${attempt} of ${retryPolicy.maxAttempts}`);
            }
        ...
        // Apply error handling after retry logic exhausted
