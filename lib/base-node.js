...
function createMealieNode(RED, configParam, options) {
    ...
    function MealieNodeInstance(config) {
        ...
        node.on('close', function(done) {
            ...
            // Wait for any active requests to complete (with timeout)
            const checkInterval = 100; // Check every 100ms
            const maxWait = 5000; // Maximum 5 seconds wait
            let waited = 0;
            
            const waitForRequests = () => {
                if (nodeState.activeRequests === 0 || waited >= maxWait) {
                    // Log if forced close due to timeout
                    if (nodeState.activeRequests > 0) {
                        node.warn(`Node ${node.id} closed with ${nodeState.activeRequests} active requests`);
                    }
                    done();
                } else {
                    waited += checkInterval;
                    setTimeout(waitForRequests, checkInterval);
                }
            };
...
if (process.env.NODE_ENV !== 'test') {
    const staleNodeCleanupInterval = setInterval(() => {
        ...
        if (cleaned > 0) {
            console.warn(`Cleaned up ${cleaned} stale nodes`);
        }
