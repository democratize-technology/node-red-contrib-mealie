...
function createMealieNode(RED, configParam, options) {
    ...
    function MealieNodeInstance(config) {
        ...
        node.on('input', async function(msg, send, done) {
            ...
            // Update node activity tracking (immutable pattern)
            const updatedNodeState = {
                ...nodeState,
                activeRequests: nodeState.activeRequests + 1,
                totalRequests: nodeState.totalRequests + 1,
                lastActivity: Date.now()
            };
            // Update the tracked state
            Object.assign(nodeState, updatedNodeState);
            ...
            try {
                ...
                // Decrement active request counter (immutable pattern)
                const finalNodeState = {
                    ...nodeState,
                    activeRequests: nodeState.activeRequests - 1
                };
                Object.assign(nodeState, finalNodeState);
            }
        });
